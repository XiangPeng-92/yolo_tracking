...
def parse_mot_results(results):
    ...
    combined_results = [
        float(re.findall("[-+]?(?:\d*\.*\d+)", f)[0]) for f in combined_results
    ]
    # pack everything in dict
    combined_results = {
        key: value for key, value in zip(["HOTA", "MOTA", "IDF1"], combined_results)
    }
...
def run_trackeval(opt):
    if opt is None:
        ...
        exp_folder_path = increment_path(
            path=exp_folder_path, sep="_", exist_ok=opt.exist_ok
        )
