...
def generate_mot_results(args):
    ...
    dets = np.loadtxt(
        args.dets_file_path, skiprows=1
...
def parse_opt():
    ...
    parser.add_argument(
        "--yolo-model", type=Path, default="yolov8n", help="yolo model path"
    )
    parser.add_argument(
        "--reid-model",
        type=Path,
    ...
    parser.add_argument("--name", default="mot", help="save results to project/name")
    parser.add_argument(
        "--dets",
        type=str,
        default="yolov8n",
...
def run_generate_mot_results(opt):
    ...
    exp_folder_path = opt.project / (
        str(opt.dets) + "_" + str(opt.embs) + "_" + str(opt.tracking_method)
    )
    exp_folder_path = increment_path(path=exp_folder_path, sep="_", exist_ok=False)
    opt.exp_folder_path = exp_folder_path
    dets_file_paths = [
        item
        for item in (opt.project.parent / "dets_n_embs" / opt.dets / "dets").glob(
            "*.txt"
        )
    ]
    embs_file_paths = [
        item
        for item in (
            opt.project.parent / "dets_n_embs" / opt.dets / "embs" / opt.embs
        ).glob("*.txt")
    ]
