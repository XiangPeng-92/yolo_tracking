    ...
    for old_type, new_type in deprecated_types.items():
        ...
        if grep_result.stdout:
            ...
            try:
                ...
                LOGGER.error(
                    f"Error occurred while trying to replace {old_type} with {new_type}: {e}"
                )
        else:
            LOGGER.info(
                f"No occurrences of {old_type} found in TrackEval to replace with {new_type}."
            )
...
def eval_setup(opt, val_tools_path):
    ...
    gt_folder = (
        mot_seqs_path  # Assuming gt_folder is the same as mot_seqs_path initially
    )
    ...
    if opt.benchmark == "MOT17":
        ...
        seq_paths = [
            p / "img1"
            for p in mot_seqs_path.iterdir()
            if p.is_dir() and "FRCNN" in str(p)
        ]
    ...
    if results.size != 0:
        if isinstance(results, np.ndarray):
            ...
            frame_indices = torch.full(
                (num_detections, 1), frame_idx + 1, dtype=torch.float32
            )
